...
from ZSI.wstools.Namespaces import SCHEMA, XMLNS, SOAP, APACHE
from ZSI.wstools.Utility import DOM, DOMException, Collection, SplitQName, basejoin
...
class SchemaReader:
    ...
    def loadFromStream(self, _file, url=None):
    ...
    def loadFromFile(self, filename):
        ...
        f = open(filename, 'rb')
        try:
            schema = self.loadFromStream(f, filename)
        finally:
            f.close()
...
class DOMAdapter(DOMAdapterInterface):
        ...
        for child in DOM.getElements(self.__node, None):
            if child.nodeType == ELEMENT_NODE and\
               SplitQName(child.tagName)[1] in contents:
                nodes.append(child)
        return [self.__class__(x) for x in nodes]
    ...
    def loadDocument(self, _file):
        self.__node = DOM.loadDocument(_file)
...
#        raise SchemaError, 'No schema item "%s" in collection %s' %(tdc, collection)
    ...
    def __setAttributeDefaults(self):
        ...
        for k,v in list(self.__class__.attributes.items()):
            if v is not None and k not in self.attributes:
...
if sys.version_info[:2] >= (2, 2):
    ...
    from ZSI.wstools import UserTuple
