...
from ZSI.wstools.TimeoutSocket import TimeoutSocket
from urllib.parse import urlparse
...
from ZSI.wstools.Namespaces import SCHEMA, SOAP, XMLNS, ZSI_SCHEMA_URI

def old_join(words, sep=' '):
...
#
# python2.3 urllib.parse.urljoin does not remove current directory ./
...
# if sys.version_info[0:2] < (2, 4, 0, 'final', 0)[0:2]:
    #basejoin = lambda base,url: urllib.parse.urljoin(base,url.lstrip('./'))
token = './'
def basejoin(base, url): 
    if url.startswith(token) is True:
        return urllib.parse.urljoin(base,url[2:])
...
class DOM:
    ...
    def getElement(self, node, name, nsuri=None, default=old_join):
        ...
        for child in node.childNodes:
            if child.nodeType == ELEMENT_NODE:
                if ((child.localName == name or name is None) and
                    ...
                    return child
        if default is not old_join:
            return default
        raise KeyError(name)

    def getElementById(self, node, _id, default=old_join):
    ...
    def getAttr(self, node, name, nsuri=None, default=old_join):
