...
from ZSI import _children, _child_elements, _floattypes, \
...
class TypeCode:
    ...
    def SimpleHREF(self, elt, ps, tag):
        ...
        href = _find_href(elt)
        if not href:
            if self.minOccurs == 0 or self.nilled(elt, ps): return None
...
class String(SimpleType):
    ...
    def __init__(self, pname=None, strip=True, **kw):
        ...
        if 'resolver' in kw: self.resolver = kw['resolver']
        self.strip = strip
...
from ZSI.schema import GTD, GED, WrapImmutable
